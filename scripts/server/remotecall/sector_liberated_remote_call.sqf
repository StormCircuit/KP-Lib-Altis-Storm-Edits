...
//edited by storm to make the enemy even more active so taking territory is more of a fight
/*
switch (true) do {
...
*/
switch (true) do {
    case (_liberated_sector in sectors_bigtown):    {_combat_readiness_increase = 15 + floor (random 10) * GRLIB_difficulty_modifier;};
    case (_liberated_sector in sectors_capture):    {_combat_readiness_increase = 10 + floor (random 6) * GRLIB_difficulty_modifier;};
    case (_liberated_sector in sectors_military):   {_combat_readiness_increase = 15 + (floor (random 11)) * GRLIB_difficulty_modifier;};
    case (_liberated_sector in sectors_factory):    {_combat_readiness_increase = 7 + (floor (random 7)) * GRLIB_difficulty_modifier;};
    case (_liberated_sector in sectors_tower):      {_combat_readiness_increase = 5 + floor (random 5);};
};
...
if (GRLIB_endgame == 0) then {
    if (
        //removed because why wouldnt they retake a tower??
        /*
        !(_liberated_sector in sectors_tower)
        && 
        */

        //storms edit: changed combat readiness check to decrease readiness to cause an attack
        // basically more action as soon as you capture your second location on max aggression
        (
            //(random (150 / (GRLIB_difficulty_modifier * GRLIB_csat_aggressivity))) < (combat_readiness - 15)
            (random (150 / (2 * GRLIB_csat_aggressivity))) < (combat_readiness)
